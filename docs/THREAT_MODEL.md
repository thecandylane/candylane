...
| T5 | Private identity key exposure | Info disclosure | ADV-3 | DPAPI (user-scope) primary on Windows — key is ciphertext on disk; successful unprotect is the load-time assertion. Simple owner-only grant on the ciphertext file as defense-in-depth. Unix 0600 fallback. Explicit export/re-wrap for author-machine key portability (Phase 5). Hardware-key option (FIDO2) later. | ✅ (model) / 🔜 P5 (FIDO2 + export) |
